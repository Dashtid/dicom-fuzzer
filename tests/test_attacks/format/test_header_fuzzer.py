...
class TestMutateBytes:
    """HeaderFuzzer.mutate_bytes wraps corrupt_random_length_field."""
    ...
    def test_no_op_path(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """When random.random() >= 0.5, mutate_bytes returns input unchanged."""
        fuzzer = HeaderFuzzer()
        data = _minimal_dicom_bytes()
        monkeypatch.setattr(
            "dicom_fuzzer.attacks.format.header_fuzzer.random.random",
            lambda: 0.99,
        )
        assert fuzzer.mutate_bytes(data) == data

    def test_mutation_path(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """When random.random() < 0.5, mutate_bytes mutates the input."""
        fuzzer = HeaderFuzzer()
        data = _minimal_dicom_bytes()
        monkeypatch.setattr(
            "dicom_fuzzer.attacks.format.header_fuzzer.random.random",
        ...
        assert isinstance(result, bytes)
        assert len(result) == len(data)  # length-preserving
        assert result[:132] == data[:132]  # preamble + DICM intact
