...
**Date:** {Path("artifacts/demo").stat().st_mtime}
...
"""
    ...
    report += "\n## Output Files\n\n"
    report += f"- **Visualizations:** `artifacts/demo/images/` ({len(list((output_dir / 'images').glob('*.png')))} images)\n"
    report += f"- **Fuzzed Files:** `artifacts/demo/fuzzed/` ({len(list((output_dir / 'fuzzed').glob('*.dcm')))} files)\n"
    report += f"- **Crashes:** `artifacts/demo/crashes/` ({len(list((output_dir / 'crashes').glob('*.dcm')))} files)\n"
    report += "- **Reports:** `artifacts/demo/reports/`\n"
