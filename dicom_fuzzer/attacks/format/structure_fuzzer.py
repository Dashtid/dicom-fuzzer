...
from dicom_fuzzer.utils.binary_mutators import (
    DATA_OFFSET as _DATA_OFFSET,
)
from dicom_fuzzer.utils.binary_mutators import (
    DICM_MAGIC as _DICM_MAGIC,
)
from dicom_fuzzer.utils.binary_mutators import (
    DICM_OFFSET as _DICM_OFFSET,
)
from dicom_fuzzer.utils.binary_mutators import (
    LONG_VRS as _LONG_VRS,
)
from dicom_fuzzer.utils.binary_mutators import (
    corrupt_random_length_field,
)
from dicom_fuzzer.utils.binary_mutators import (
    parse_dicom_elements as _parse_dicom_elements,
)
...
class StructureFuzzer(FormatFuzzerBase):
    ...
    # ------------------------------------------------------------------
    # Length-corruption patterns (CORRUPT_LENGTHS_4B / 2B) and the
    # length-field rewriter live in dicom_fuzzer.utils.binary_mutators
    # so other format fuzzers can opt into the attack class without
    # duplicating the byte walker.
    ...
    def _binary_corrupt_length_field(self, file_data: bytes) -> bytes:
        ...
        Thin wrapper over
        :func:`dicom_fuzzer.utils.binary_mutators.corrupt_random_length_field`,
        which is the canonical implementation now shared with HeaderFuzzer,
        MetadataFuzzer, and PrivateTagFuzzer.
        """
        return corrupt_random_length_field(file_data)
