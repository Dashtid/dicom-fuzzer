...
Examples:
  ...
  python coverage_fuzz.py --dicom-aware --adaptive
    ...
    target_group.add_argument(
        "--target",
        type=str,
        help="Path to target binary or Python module"
    )
    target_group.add_argument(
        "--target-args",
        type=str,
        help="Arguments to pass to target binary"
    )
    target_group.add_argument(
        "--modules",
        type=str,
        nargs="+",
        help="Python modules to track coverage for"
    )
    ...
    coverage_group.add_argument(
        "--no-coverage",
        action="store_true",
        help="Disable coverage-guided fuzzing"
    )
    coverage_group.add_argument(
        "--branches",
        action="store_true",
    ...
    output_group.add_argument(
        "--save-all",
        action="store_true",
        help="Save all generated inputs"
    )
    output_group.add_argument(
        "--report-interval",
        type=int,
...
def create_status_table(stats: dict) -> Table:
    ...
    table.add_row("Crashes", f"{stats.get('total_crashes', 0)} ({stats.get('unique_crashes', 0)} unique)")
    ...
    if args.dry_run:
        console.print(Panel(
            json.dumps({
                ...
                'corpus_dir': str(config.corpus_dir) if config.corpus_dir else None,
