...
class GrammarRule:
    ...
    def generate(
        ...
        handlers = {
            RuleType.TERMINAL: self._generate_terminal,
            RuleType.SEQUENCE: self._generate_sequence,
            RuleType.CHOICE: self._generate_choice,
            RuleType.OPTIONAL: self._generate_optional,
            RuleType.REPEAT: self._generate_repeat,
            RuleType.REPEAT_PLUS: self._generate_repeat,
        }

        handler = handlers.get(self.rule_type)
        if handler:
            return handler(grammar, depth, max_depth)
        return b""

    def _generate_terminal(
        self, grammar: DICOMGrammar, depth: int, max_depth: int
    ) -> bytes:
        """Generate terminal rule output."""
    ...
    def _generate_choice(
        ...
        """Generate choice rule output with weighted selection."""
        ...
        return child_rule.generate(grammar, depth + 1, max_depth) if child_rule else b""

    def _generate_optional(
        self, grammar: DICOMGrammar, depth: int, max_depth: int
    ) -> bytes:
        """Generate optional rule output."""
    ...
    def _generate_repeat(
        self, grammar: DICOMGrammar, depth: int, max_depth: int
    ) -> bytes:
        """Generate repeat/repeat_plus rule output."""
        result = b""
