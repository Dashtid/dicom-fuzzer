...
class _StackInfo:
    ...
    has an analyzable dump; left None when DOTNET_DbgEnableMiniDump
    was not active (no dump_path on the crash record) or when ClrMD
...
def _collect_stack_info(crash: CrashRecord) -> _StackInfo | None:
    """Run the dump analyzer + stack hasher for one crash.

    Lazy-imports so pythonnet + ClrMD only load when a campaign
    actually has dumps to render. Returns None on any failure path
    so callers can just skip the stack section.
    """
...
def _write_one_cluster(
    ...
    if stack_info is not None:
        ...
        lines.append("")
        lines.append(f"- **backend**: `{stack_info.backend}`")
